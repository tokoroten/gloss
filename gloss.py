...
class GlossGame(object):
    ...
    def run(self):
        ...
        while Gloss.game_is_running:
            Gloss.elapsed_seconds = Gloss.game_clock.tick(60) / 1000.0
...
class Gloss(object):
    ...
    @staticmethod
    def draw_pie(position = (0,0), radius = 100, angle=(0.0, 360.0), corner=16, color = Color.WHITE, is_edge=False, width=3):
        glPushMatrix()
        ...
        for i in xrange(corner + 1):
            rad = ((angle[0] + (angle[1] - angle[0])/corner * i) ) / 180.0 * math.pi 
    ...
    @staticmethod
    def draw_arc(position = (0,0), max_radius = 100, min_radius = 50, angle=(0.0, 360.0), corner=16, color = Color.WHITE, is_edge=False, width=3):
        glPushMatrix()
        ...
        for i in xrange(corner + 1):
            ...
            x = math.cos(rad) * max_radius
            y = math.sin(rad) * max_radius
            glVertex2f(x, y)

            x = math.cos(rad) * min_radius
            y = math.sin(rad) * min_radius
            glVertex2f(x, y)
...
class Texture(object):
    ...
    def draw(self, position = (0, 0), rotation = 0.0, origin = (0, 0), scale=None, scale_x = 1, scale_y = 1, color = Color.WHITE):
        ...
        texwidth = self.width * scale_x
        texheight = self.height * scale_y

        if origin is None:
            originx = texwidth / 2
            originy = texheight / 2
        else:
            originx = origin[0] * scale_x
            originy = origin[1] * scale_y
...
class ParticleSystem(object):
    ...
    def __init__(self, texture, position = None, lifespan = -1, creationspeed = None, initialparticles = 50, particlelifespan = 1000, minspeed = 50, maxspeed = 250, minrotation = 0, maxrotation = 0, minscale = 1.0, maxscale = 1.0, growth = 0.0, wind = None, drag = None, startcolor = Color.WHITE, endcolor = Color.TRANSPARENT_WHITE, onfinish = None, name = "", scale_x_rate = 1.0):
        ...
        self.scale_x_rate = scale_x_rate
    ...
    def draw(self):
        ...
        for particle in self.particles:
            scale = particle.scale + (particle.anim_pos * self.particle_growth)
            scale_x = scale * self.scale_x_rate
            scale_y = scale
            self.texture.draw(position = particle.position, rotation = particle.rotation, origin = None, scale_x = scale_x, scale_y = scale_y, color = particle.color)
    ...
    def update(self):
        ...
        for i in reversed(xrange(len(self.particles))):
...
#        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F_ARB, width, height, 0, GL_RGBA, GL_FLOAT, None)
        glTexImage2D(GL_TEXTURE_2D, 0, 0x8814, width, height, 0, GL_RGBA, GL_FLOAT, None)
        ...
        for line in lines:
            for letter in line:
                if letter not in self.characters:
                    self.characters[letter] = SpriteFontLetter(self.font, letter)
